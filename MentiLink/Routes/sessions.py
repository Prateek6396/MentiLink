...
def my_sessions():
    ...
    if current_user.role == 'mentor':
        query = Session.query.filter_by(mentor_id=current_user.id)
        page_title = "My Mentoring Sessions"
    else:
        query = Session.query.filter_by(student_id=current_user.id)
        page_title = "My Learning Sessions"

    # Apply filter if provided
    if status_filter in ['scheduled', 'completed', 'cancelled']:
        query = query.filter_by(status=status_filter)
        page_title += f" ({status_filter.capitalize()})"

    sessions = query.all()
    
    return render_template('my_sessions.html', sessions=sessions, page_title=page_title)

@sessions_bp.route('/session/<int:session_id>/cancel', methods=['POST'])
@login_required
def cancel_session(session_id):
    ...
    if current_user.id not in [session_obj.mentor_id, session_obj.student_id]:
        flash('You are not authorized to cancel this session.', 'danger')
        return redirect(url_for('dashboard.dashboard'))

    # Identify both users before making changes
    mentor = session_obj.mentor
    student = session_obj.student
    session_time = session_obj.slot.start_time.strftime('%Y-%m-%d at %H:%M')

    # Change session status and make the slot available again
    session_obj.status = 'cancelled'
    session_obj.slot.is_available = True
    
    db.session.commit()
    
    flash('Session has been cancelled successfully.', 'success')
    ...
    try:
        ...
        msg_student.body = (
            ...
            f"Your session with {mentor.full_name} scheduled for {session_time} has been cancelled by {current_user.full_name}.\n\n"
            f"The time slot is now available again for booking.\n\n"
            f"Regards,\nMentiLink Team"
        )
        ...
        msg_mentor.body = (
            ...
            f"Your session with {student.full_name} scheduled for {session_time} has been cancelled by {current_user.full_name}.\n\n"
            f"This time slot has been made available again on your schedule.\n\n"
            f"Regards,\nMentiLink Team"
        )
        mail.send(msg_mentor)
    except Exception as e:
        flash(f"Session cancelled, but failed to send email notification: {e}", "warning")
